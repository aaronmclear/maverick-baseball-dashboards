...
const headerAliases = {
  ...
  gb_percent: 'gbPct',
  babip: 'babip',
  pitching_gp: 'pitchingGp',
  gp_pitched: 'pitchingGp',
  games_pitched: 'pitchingGp',
  ip_outs: 'ipOuts',
  ipouts: 'ipOuts',
  bf: 'bf',
  batters_faced: 'bf',
...
const numericFields = [
  ...
  'pitchingGp', 'ipOuts', 'pitches', 'bf', 'strikePct', 'firstPitchStrikePct', 'hAllowed', 'r', 'er', 'bb', 'kLooking', 'wins', 'losses', 'abAgainst',
  'tc', 'a', 'po', 'e', 'dp'
];
...
function hasStats(bucket) {
  if (!bucket) return false;
  const hitting = bucket.hitting || {};
  const pitching = bucket.pitching || {};
  const fielding = bucket.fielding || {};
  return Boolean(
    (Number(hitting.pa) || 0) ||
    (Number(hitting.ab) || 0) ||
    (Number(pitching.bf) || 0) ||
    (Number(pitching.ipOuts) || 0) ||
    (Number(fielding.tc) || 0)
  );
}

function addSelectBuckets(base, incoming) {
  return {
    hitting: mergeHittingStats(base.hitting || blankHitting(), incoming?.hitting || blankHitting()),
    pitching: mergePitchingStats(base.pitching || blankPitching(), incoming?.pitching || blankPitching()),
    fielding: sumStats(base.fielding || blankFielding(), incoming?.fielding || blankFielding())
  };
}
...
function buildSelectTotalRow(rows) {
  ...
  if (state.family === 'pitching') {
    const playerPpos = rows
      .map(row => {
        const gp = Number(row.pitching?.gp) || 0;
        const pitches = Number(row.pitching?.pitches) || 0;
        return gp > 0 ? pitches / gp : NaN;
      })
      .filter(Number.isFinite);

    totalRow.pitching.ppoOverride = playerPpos.length
      ? playerPpos.reduce((sum, value) => sum + value, 0) / playerPpos.length
      : NaN;
  }
...
function overrideImportedRows(rows, target) {
  ...
  return rows.map(row => {
    ...
    if (target.source === 'select') {
      ...
      next.selectTeamId = target.teamId || 'spring-league';
...
function inferSourceFromFilename(name) {
  ...
  if (lower.includes('fall')) {
    return { type: 'select', label: 'Fall League', teamId: 'fall-league' };
  }
  if (lower.includes('spring') || lower.includes('maroon')) {
    return { type: 'select', label: 'Spring League', teamId: 'spring-league' };
  }
...
function mergeImportedRows(rows) {
  ...
  for (const rawRow of rows) {
    ...
    const pitching = {
      gp: row.pitchingGp || 0, ipOuts: row.ipOuts, pitches: row.pitches, bf: row.bf || 0, strikePct: row.strikePct || 0, firstPitchStrikePct: row.firstPitchStrikePct || 0, hAllowed: row.hAllowed, r: row.r || 0, er: row.er, bb: row.bbAllowed || row.bb || 0,
...
function aggregateSelectRowsForPreview(rows, existingData, teamId = 'spring-league') {
...
function wireEvents() {
  document.addEventListener('click', async event => {
    ...
    if (selectViewButton) {
      ...
      state.selectTeamId = 'spring-league';
      renderPage();
      return;
    }
